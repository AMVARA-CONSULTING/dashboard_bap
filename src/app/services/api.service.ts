...
export class ApiService {
  ...
  getSavedReportData(reportKey: ReportTypes): Observable<any[]> {
    ...
    if (this.corpintra) {
      ...
      return this.http.get<any>(`${config.apiLink}objects/${reportInfo.id}/versions`).pipe(
        catchError(err => {
        ...
        switchMap(json => {
          ...
          const mimeType = json.data[0]._meta.links.content.mimeType;
          if (mimeType) {
            // Report has mimeType
            const nextLink = json.data[0]._meta.links.content.url;
            this.reportDates[reportKey] = json.data[0].modificationTime;
            return this.http.get(nextLink, { responseType: 'text' });
          } else {
            // Report doesn't have mimeType, it means we have to use second way of get it
            const id = json.data[0].id;
            return this.http.get<any>(`${config.apiLink}objects/${id}/items`).pipe(
              catchError(err => {
                // Catch error on getting first items
                console.log(`${reportKey} - Fail at getting first items`);
                return throwError(err);
              }),
              switchMap(data => this.http.get<any>(`${config.apiLink}objects/${data.data[0].id}/items`)),
              catchError(err => {
                // Catch error on getting second items
                console.log(`${reportKey} - Fail at getting second items`);
                return throwError(err);
              }),
              switchMap(data => this.http.get(`${config.apiLink}disp/repository/sid/cm/oid/${data.data[0].id}/content`, { responseType: 'text' }))
            );
          }
        }),
        catchError(err => {
        ...
        map(html => {
          ...
          if (columns.length > 0) {
            rows = rows.map((row, index) => {
              columns.forEach(num => {
                row[index][num] = isNaN(row[index][num]) ? 0 : parseFloat(row[index][num]);
              });
              return row;
            });
          }
